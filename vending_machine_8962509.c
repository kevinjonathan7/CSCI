...
int main() {
    ...
    while (1) {
        ...
        if (choice == 0) break;                      // exit program completely
        else if (choice == 1) purchase_product();   // call purchase_product()
        else if (choice == 2) admin_mode();         // call admin_mode()
        else printf("Invalid option!\n");           // invalid menu option
    }

    printf("Exited successfully.\n");                // final exiting message
